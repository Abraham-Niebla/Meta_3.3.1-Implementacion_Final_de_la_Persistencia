...
const getById = async function (req, res) {
    try {
        let p = await tipoDeActivoModel.findByPk(req.params.id); //Busca por llave primaria (id)
...
const deleteId = async function (req, res) {
    try {
        ...
        if (p.length === 0) {
            ...
            await tipoDeActivoModel.destroy({ where: { id: req.params.id } }); //Elimina un Activo por su id
            await res.json(p);
        }
    } catch (err) {
        await res
